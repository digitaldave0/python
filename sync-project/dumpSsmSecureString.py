...
def dump_secure_string_parameters_to_json(region, output_folder):
        ...
        try:
            ...
            for param in response.get('Parameters', []):
                if param['Type'] == 'SecureString':
                    # Fetch parameter metadata to get last modified date
    ...
    output_file = os.path.join(output_folder, 'secure_string_parameters.json')

    # Write parameters to JSON file
    with open(output_file, 'w') as f:
        json.dump(parameters, f, indent=4)

    print(f"Secure string parameters dumped to {output_file}")
...
if __name__ == '__main__':
    ...
    # Dump secure string parameters to the specified folder
    dump_secure_string_parameters_to_json(args.region, args.output_folder)
